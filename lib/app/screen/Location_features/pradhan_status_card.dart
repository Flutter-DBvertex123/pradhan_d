...
class PradhanStatusCard extends StatefulWidget {
  const PradhanStatusCard({
    super.key,
  ...
  @override
  State<PradhanStatusCard> createState() => _PradhanStatusCardState();
}

class _PradhanStatusCardState extends State<PradhanStatusCard> {
  final WelcomeLocationController welcomeController =
      Get.find<WelcomeLocationController>();
  @override
  void initState() {
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Obx(
        // () => (welcomeController.pradhanId.value.isEmpty ||
        //             welcomeController.pradhanId.value !=
        //                 getPrefValue(Keys.USERID)) &&
        //         (welcomeController.pradhanStatus.value.isEmpty &&
        //             welcomeController.pradhanId.value !=
        //                 getPrefValue(Keys.USERID))
        () {
      // if the current user is pradhaan
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                        ...
                        : CircleAvatar(
                            ...
                            child: ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                ...
                                imageUrl:
                                    welcomeController.pradhanModel.value.image,
                                // .replaceAll('\', '//'),
                                fit: BoxFit.cover,
                                // width: 160.0,
                                height: 160.0,
                    ...
                    SizedBox(
                        width:
                            welcomeController.pradhanId.value.isEmpty ? 0 : 8),
                    Expanded(
                      child: Text(
                        welcomeController.pradhanStatus.value != ""
                            ? welcomeController.pradhanStatus.value
                            : isCurrentUserPradhaan
                                ? 'Hello ${welcomeController.pradhanModel.value.name} please set status'
                                : 'No Pradhaan Updates',
                        maxLines: null,
                        textAlign: welcomeController.pradhanId.value.isEmpty
                            ? TextAlign.center
                            : TextAlign.left,
                        style: TextStyle(
                            fontStyle:
